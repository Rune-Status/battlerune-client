...
public class AudioInstrument {
	...
	final int[] synthesize(final int int_0, final int int_1) {
		Class78.method448(samples, 0, int_0);
		if (int_1 < 10) {
			return samples;
		} else {
					...
					if (oscillatorVolume[int_11] != 0) {
						...
						if (int_12 < int_0) {
							samples[int_12] += evaluateWave(phases[int_11], (volumeSteps[int_11] * int_10) >> 15,
									pitch.form);
							phases[int_11] += pitchBaseSteps[int_11] + ((pitchSteps[int_11] * int_9) >> 16);
						}
					}
				}
			}
			...
			if (release != null) {
				...
				for (int_11 = 0; int_11 < int_0; int_11++) {
					...
					if (bool_1) {
						int_9 = (((release.end - release.start) * int_12) >> 8) + release.start;
					} else {
	...
	final void decode(final Buffer buffer_0) {
		...
		if (int_0 != 0) {
			...
			anAudioEnvelope1 = new AudioEnvelope();
